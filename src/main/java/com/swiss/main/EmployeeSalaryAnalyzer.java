	...
	public static void validateLongReportingLine(Map<Integer, EmployeeEntity> employeeMap, EmployeeEntity ceo) {
		for (EmployeeEntity employeeEntity : employeeMap.values()) {
            ...
            if (reportingLineLength > 4) {
                System.out.println(employeeEntity.getFirstName() + " " + employeeEntity.getLastName() + " has a reporting line too long by " + (reportingLineLength - 4) + " managers.");
            }
        }
	}
		...
		for (EmployeeEntity manager : employeeMap.values()) {
            if (manager.getManagerId()!= null) {
                ...
                if (manager.getSalary() < 1.2 * avgSalary) {
                    System.out.println(manager.getFirstName() + " " + manager.getLastName() + " earns less than required by " + (1.2 * avgSalary - manager.getSalary()));
                }
                //managers earn more than they should, and by how much
                if (manager.getSalary() > 1.5 * avgSalary) {
                    System.out.println(manager.getFirstName() + " " + manager.getLastName() + " earns more than required by " + (manager.getSalary() - 1.5 * avgSalary));
                }
            }
        }
	}
